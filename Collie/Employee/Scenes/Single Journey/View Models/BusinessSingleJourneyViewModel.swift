...
final class BusinessSingleJourneyViewModel: ObservableObject {
    ...
    func saveEvent(_ event: Event, completion: ([Event]) -> ()) {
...
//            sortJourneyEvents()
        ...
        completion(self.business.events)
        objectWillChange.send()
    }
    
    func removeEvent(_ event: Event, completion: ([Event]) -> ()) {
        if let index = self.business.events.firstIndex(where: {$0.id == event.id}) {
    ...
    func duplicateEvent(_ event: Event, completion: ([Event]) -> ()) {

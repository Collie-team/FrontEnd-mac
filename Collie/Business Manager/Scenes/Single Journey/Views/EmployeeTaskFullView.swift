...
struct EmployeeTaskFullView: View {
    @ObservedObject var employeeSingleJournerViewModel: EmployeeSingleJourneyViewModel
    ...
    var body: some View {
        VStack(spacing: 0) {
            VStack(alignment: .leading, spacing: 15) {
                ...
                Text(employeeSingleJournerViewModel.chosenTaskModel?.task.taskCategory?.name ?? "Sem categoria")
                    .padding(.horizontal, 8)
                    .padding(.vertical, 4)
                    .background(employeeSingleJournerViewModel.chosenTaskModel?.task.taskCategory?.color ?? .gray)
                    .cornerRadius(16)
        ...
        .background(
            HStack(spacing: 0) {
                Rectangle()
                    .foregroundColor(employeeSingleJournerViewModel.chosenTaskModel?.task.taskCategory?.color ?? .gray)
                    .frame(width: 30)
                ZStack {
                    Rectangle()
